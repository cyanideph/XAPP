...
import {
  createContentCategory, getEngagementLeaderboard, listContentCategories, listHiddenContent,
...
export default function ContentAdminScreen() {
  ...
  const [name,setName]=useState(''), [slug,setSlug]=useState(''), [kind,setKind]=useState('post'), [busy,setBusy]=useState(true), [error,setError]=useState('');
  async function load(){setBusy(true);setError('');try{const [a,b,c]=await Promise.all([listContentCategories(),listHiddenContent(50,0),getEngagementLeaderboard('all',25,0)]);setCategories(a);setHidden(b);setLeaders(Array.isArray(c)?c:[]);}catch(e){setError(e instanceof Error?e.message:'Unable to load administration.')}finally{setBusy(false)}}
  useEffect(()=>{void load()},[]);
