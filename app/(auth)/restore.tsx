...
export default function RestoreScreen() {
  ...
  useEffect(() => {
    ...
    async function prepare(url?: string) {
      try {
        ...
        if (mounted) {
          setError(e instanceof Error ? e.message : 'Unable to restore the secure authentication session.');
          setReady(true);
        }
      }
    }
  ...
  if (!ready) {
    return (
      <YStack flex={1} p="$5" style={{ justifyContent: 'center', alignItems: 'center' }} bg="$background">
  ...
  return (
    <YStack flex={1} p="$5" style={{ justifyContent: 'center' }} gap="$4" bg="$background">
      ...
      <YStack gap="$3">
        ...
        <Button onPress={() => { void updatePassword(); }} disabled={busy || !password || !confirmPassword}>
