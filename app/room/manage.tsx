...
export default function ManageRoomScreen() {
  ...
  return <YStack flex={1} p="$4" pt="$7" gap="$4" bg="$background">
    ...
    <YStack gap="$2">
      ...
      {reports.map(report => <YStack key={report.id} gap="$2" p="$3" borderWidth={1} borderColor="$borderColor">
        ...
        <XStack gap="$2" flexWrap="wrap">
          {report.message_id && report.status !== 'resolved' && report.status !== 'dismissed' ? <Button size="$2" disabled={busy !== null} onPress={() => { void run('delete-report-' + report.id, async () => { await moderateReportedMessage(report.message_id!); await updateRoomReport(report.id, 'resolved', 'Message removed after report review'); }); }}>Remove message</Button> : null}
          {report.status !== 'resolved' && report.status !== 'dismissed' ? <Button size="$2" disabled={busy !== null} onPress={() => { void run('resolve-' + report.id, () => updateRoomReport(report.id, 'resolved', 'Reviewed by Room staff')); }}>Resolve</Button> : null}
          {report.status !== 'resolved' && report.status !== 'dismissed' ? <Button size="$2" chromeless disabled={busy !== null} onPress={() => { void run('dismiss-' + report.id, () => updateRoomReport(report.id, 'dismissed', 'Dismissed by Room staff')); }}>Dismiss</Button> : null}
        </XStack>
      </YStack>)}
    ...
    <YStack gap="$2">
      ...
      {banned.map(member => <XStack key={member.user_id} gap="$2" style={{ alignItems: 'center', justifyContent: 'space-between' }}><Text flex={1}>{member.nickname || member.user_id}</Text><Button size="$2" disabled={busy !== null} onPress={() => { void run('unban-' + member.user_id, () => unbanRoomMember(id, member.user_id)); }}>Unban</Button></XStack>)}
    ...
    <YStack gap="$2">
      ...
      {muted.map(member => <XStack key={member.user_id} gap="$2" style={{ alignItems: 'center', justifyContent: 'space-between' }}><Text flex={1}>{member.nickname || member.user_id}</Text><Button size="$2" disabled={busy !== null} onPress={() => { void run('unmute-' + member.user_id, () => moderateRoomMember(id, member.user_id, 'mute', 0, 'Unmuted by Room staff')); }}>Unmute</Button></XStack>)}
    ...
    <YStack gap="$2">
      ...
      {visibleMembers.map(member => { const label = member.nickname || member.profile?.display_name || member.profile?.username || member.user_id; const coHost = coHosts.has(member.user_id); return <YStack key={member.user_id} gap="$2" p="$2" borderWidth={1} borderColor="$borderColor"><XStack style={{ alignItems: 'center', justifyContent: 'space-between' }}><YStack flex={1}><Text fontWeight="800">{label}</Text><Text fontSize="$2" color="$colorPress">@{member.profile?.username || 'unknown'} · {member.role}{coHost ? ' · co-host' : ''}</Text></YStack></XStack><XStack gap="$2" flexWrap="wrap"><Button size="$2" disabled={busy !== null || member.role === 'owner'} onPress={() => { void run('cohost-' + member.user_id, async () => { const enabled = !coHost; await setRoomCoHost(id, member.user_id, enabled); }); }}>{coHost ? 'Remove co-host' : 'Make co-host'}</Button><Button size="$2" disabled={busy !== null || member.role === 'owner'} onPress={() => { void run('warn-' + member.user_id, () => warnRoomMember(id, member.user_id)); }}>Warn</Button><Button size="$2" disabled={busy !== null || member.role === 'owner'} onPress={() => { void run('mute-' + member.user_id, () => moderateRoomMember(id, member.user_id, 'mute', 60, 'Room moderation')) }}>Mute</Button><Button size="$2" disabled={busy !== null || member.role === 'owner'} onPress={() => { void run('ban-' + member.user_id, () => moderateRoomMember(id, member.user_id, 'ban', 1440, 'Room moderation')) }}>Ban</Button></XStack></YStack>; })}
    </YStack>
