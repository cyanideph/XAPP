...
export default function CreateRoomScreen() {
  ...
  return <YStack flex={1} p="$5" pt="$8" gap="$4" bg="$background"><Text fontSize="$3" color="$colorPress" fontWeight="800">ROOMS</Text><H1 fontSize="$9">Create a Room</H1><Paragraph color="$colorPress">Create the Room first, then automatically join it as the creator.</Paragraph><YStack gap="$3"><Text fontSize="$5" fontWeight="800">Room identity</Text><Paragraph color="$colorPress">Create a focused community space.</Paragraph><Input value={name} onChangeText={value=>{setName(value);if(!slug)setSlug(slugify(value));}} placeholder="Room name"/><Input value={slug} onChangeText={setSlug} placeholder="Slug" autoCapitalize="none"/><Input value={provinceCode} onChangeText={setProvinceCode} placeholder="Province code (optional)" autoCapitalize="characters"/><Input value={description} onChangeText={setDescription} placeholder="Description (optional)" multiline/></YStack>{error?<Paragraph color="$red10">{error}</Paragraph>:null}<XButton size="$4" icon={<XIcon name="plus" size={18} color="#FFFFFF" />} disabled={busy} onPress={()=>{void submit();}}>{busy?'Creating…':'Create Room'}</XButton><XButton chromeless icon={<XIcon name="close" size={18} color="#8B85FF" />} onPress={()=>router.back()}>Cancel</XButton></YStack>;
}
