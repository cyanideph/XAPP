...
import { joinRoom, leaveRoom, listOnlineRoomMembers, touchRoomPresence } from '../../src/features/chat/backend';
...
export default function RoomScreen() {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        await touchRoomPresence(id);
        const rows = await listOnlineRoomMembers(id, 20, 0);
        if (active) setOnlineMembers(Array.isArray(rows) ? rows : []);
      } catch {}
    };

    void joinRoom(id).then(() => refresh());
  ...
  return <YStack flex={1} bg="$background">
    <YStack p="$4" borderBottomWidth={1} borderColor="$borderColor">
      <H1 fontSize="$7">Room</H1>
      <Text fontSize="$2" color="$colorPress">{onlineMembers.length} recently active</Text>
