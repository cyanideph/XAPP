...
import { getNotificationPreferences, setNotificationPreferences } from '../../src/lib/backend';
...
export default function Settings(){
 const [p,setP]=useState<Record<string,boolean>|null>(null),[s,setS]=useState(''),[email,setEmail]=useState(''),[current,setCurrent]=useState(''),[next,setNext]=useState(''),[confirm,setConfirm]=useState(''),[busy,setBusy]=useState(false);
 useEffect(()=>{void getNotificationPreferences().then(x=>setP(x as Record<string,boolean>)).catch(e=>setS(e instanceof Error?e.message:'Unable to load settings.')); void supabase?.auth.getUser().then(({data})=>setEmail(data.user?.email??''));},[]);
